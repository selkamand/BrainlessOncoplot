...
export function yAxisLayout() {
  ...
  my.computeLayout = function () {
    ...
    if (geneBarPadding === undefined)
      throw new Error("geneBarPadding is undefined");
    if (geneBarWidth === undefined)
      throw new Error("geneBarWidth is undefined");
    if (tickMarkAndTextPadding === undefined)
      throw new Error("tickMarkAndTextPadding is undefined");
    ...
    metrics.yTextAndTickWidth =
      maxCharactersDomain * fontSizeDomain + Math.abs(tickWidth);

    // Calculate X pixel position of Y axis
    metrics.oncoplotPosStartX =
      margin.left + metrics.facetWidth + metrics.yTextAndTickWidth;
    metrics.oncoplotWidth =
      windowWidth -
      margin.left -
      margin.right -
      metrics.facetWidth -
    ...
    metrics.geneBarPosStartX =
      metrics.oncoplotPosStartX + metrics.oncoplotWidth + geneBarPadding;
...
export function xAxisLayout() {
  ...
  my.oncoplotClinicalPadding = function (_) {
    return arguments.length
      ? ((oncoplotClinicalPadding = _), my)
      : oncoplotClinicalPadding;
  };
  ...
  my.computeLayout = function () {
    ...
    if (windowHeight === undefined)
      throw new Error("windowHeight is undefined");
    if (tmbBarPadding === undefined)
      throw new Error("tmbBarPadding is undefined");
    if (tmbBarHeight === undefined)
      throw new Error("tmbBarHeight is undefined");
    if (tickMarkAndTextPadding === undefined)
      throw new Error("tickMarkAndTextPadding is undefined");
    if (tickLength === undefined) throw new Error("tickLength is undefined");
    if (domain === undefined) throw new Error("domain is undefined");
    ...
    if (!showSampleNames) {
      ...
      metrics.maxSampleLabelsHeight =
        fontsizeX * maxCharactersDomain +
        Math.abs(tickLength + tickMarkAndTextPadding);
    }

    metrics.tmbBarPosStartY = margin.top;
    metrics.tmbBarHeight = tmbBarHeight;
    metrics.tmbBarPosEndY = margin.top + tmbBarHeight;
    metrics.oncoplotPosStartY =
      metrics.tmbBarPosStartY + metrics.tmbBarHeight + tmbBarPadding;
    metrics.clinicalHeight =
      clinicalRowHeight * clinicalRowNumber +
      clinicalRowPadding * (clinicalRowNumber - 1);
    metrics.oncoplotHeight =
      windowHeight -
      tmbBarHeight -
      metrics.clinicalHeight -
      metrics.maxSampleLabelsHeight -
      ...
      margin.top -
      margin.bottom;
    metrics.oncoplotPosEndY =
      metrics.oncoplotPosStartY + metrics.oncoplotHeight;
